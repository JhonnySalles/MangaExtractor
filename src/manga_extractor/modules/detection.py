...
class TextDetection():
    ...
    def text_detect(self, img, ele_size=(8, 2)):
        ...
        # same as default,None,3,1,0,cv2.BORDER_DEFAULT)
        ...
        res = cv2.findContours(
            img_threshold_morp, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        ...
        RectP = [(max(int(i[0]-noiseSizeParam), 0), max(int(i[1]-noiseSizeParam), 0), min(int(i[0]+i[2]+noiseSizeParam),
                  img.shape[1]), min(int(i[1]+i[3]+noiseSizeParam), img.shape[0])) for i in Rect]  # with padding, box  x1,y1,x2,y2
