...
import os
import manga_extractor.utils.helpers as util
import manga_extractor.modules.cleaning as clean
...
class RemoveFurigana():
    ...
    def intersects_other_component(self, row, col, component, components):
        for c in components:
            if c is component:
                continue
            if self.is_in_component(row, col, c):
                return c
        return None
    ...
    def find_cc_to_left(self, component, components, max_dist=20):
        ...
        for col in reversed(range(int(left_col), int(c_col))):
            c = self.intersects_other_component(
                c_row, col, component, components)
            if c is not None:
    ...
    def estimate_furigana(self, img, segmentation):
        ...
        columns = clean.binarize(
            columns, threshold=defaults.FURIGANA_BINARY_THRESHOLD)
    ...
    def removeFurigana(self, imgPath, imgGray, imgClean, imgSegment, outputTextOnlyPath, outputFuriganaPath):
        fileName = os.path.basename(imgPath)
        furigana_areas = self.estimate_furigana(imgGray, imgSegment)
        cv2.imwrite(outputFuriganaPath + fileName + '_furigana.png', furigana_areas)

        furigana_mask = np.array(furigana_areas == 0, 'B')
        cleaned = cv2.bitwise_not(cv2.bitwise_not(imgClean)*furigana_mask)
        cv2.imwrite(outputFuriganaPath + fileName, cleaned)
