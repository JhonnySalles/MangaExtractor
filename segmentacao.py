...
##############################################################################
  ...
  def segment_image(self, img, max_scale=defaults.CC_SCALE_MAX, min_scale=defaults.CC_SCALE_MIN):
      ...
      (text_like_areas, nontext_like_areas) = self.filter_text_like_areas(img, segmentation=text_only, average_size=average_size)
      if arg.boolean_value('verbose'):
        print('**********there are ' + str(len(text_like_areas)) + ' text like areas total.')
      text_only = np.zeros(img.shape)
      util.draw_bounding_boxes(text_only, text_like_areas,color=(255),line_size=-1)
...
## Comentado a chamada, verificar se irá ser necessário
  ...
  def segmentFurigana(self, imgPath, outputFuriganaPath):
      ...
      inv_binary = cv2.bitwise_not(clean.binarize(gray, threshold=defaults.BINARY_THRESHOLD))
      binary = clean.binarize(gray, threshold=defaults.BINARY_THRESHOLD)

      segmentacao = TextSegmenation(self.operacao)
      segmented_image = segmentacao.segment_image(gray)
      ...
      cv2.imwrite(outputFuriganaPath + fileName + 'segmentacao.png', segmentation_mask)
